...
def Confirm(request):
    ...
    try:
        ...
        marcado.save()
        livre.save()
        obs = "Já tinha sido selecionado um horário para seu número de matrícula, então, ele foi liberado e o novo horário foi alocado"
        
    except Exception as ex:
        return render(request,'failed.html',{"failed":"Erro inesperado, por favor, tente novamente. Caso não consiga, entre em contato com o Pedro","ex":ex})
    ...
    html = render_to_string('code.html',{"opcao":opcao,"obs":obs,"code":val,'hora':hora,'dia':dia,'nome':post['nome'],'matricula':matricula})
    pdf = HTML(string=html).write_pdf()
    with open('confirmacao.pdf', 'wb') as arq:
        arq.write(pdf)
    return FileResponse(open('confirmacao.pdf', 'rb'))
...
def ValidaCodigo(request):
    ...
    if 'matricula' in request.GET:
        ...
        val = str(khash(mystr).hexdigest())[:30]
        resp = f"Código {'válido' if val == codigo else 'inválido'}"
    return render(request,'valida.html', {'resp':resp})
